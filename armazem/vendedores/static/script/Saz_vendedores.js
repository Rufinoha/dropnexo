(function () {
  ...
  async function carregarDetalhe(id) {
    ...
    abrir();
    const r = await fetch("/armazem/vendedores/detalhe/" + id, { credentials: "same-origin" });
  ...
  async function responder(acao, mensagem) {
    ...
    const r = await fetch("/armazem/vendedores/responder", {
      method: "POST",
  ...
  async function carregar() {
    const r = await fetch("/armazem/vendedores/dados", { credentials: "same-origin" });
    const j = await r.json();
    if (!j.success) {
